...
- App 提供 `识别截图并记账` App Intent。
- Intent 参数：`image: IntentFile`、`capturedAt: Date?`、`preferredBook: BookEntity?`、`autoSave: Bool?`。
- Intent 返回：状态、交易 ID、展示摘要、是否需要确认、可打开的深链。
- App Shortcut 的展示名称为“识别付款截图”，可被快捷指令、Siri、Spotlight、Action Button 等系统入口发现。
...
2. 可选：转换图像为 JPEG、最长边 1600 px、质量约 75%。
3. 运行 Daisy 的“识别付款截图”动作，并把上一步图像作为输入。
4. 根据返回结果显示通知：`已记账 ¥金额 · 分类`、`已保存待确认`或错误原因。
5. 默认不额外保存图像；若系统动作已经写入相册，可提供“识别成功后删除本次截图”的可选模板，但首次启用必须明确告知并验证系统是否要求确认。
