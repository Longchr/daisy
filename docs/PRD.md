...
| 条件 | 行为 |
|---|---|
| 普通账单总体置信度 ≥ 用户阈值（默认 0.90），且无本地硬冲突 | 自动入账；模型普通提示不单独阻断 |
...
- 模型必须支持视觉输入。模型列表接口通常不声明视觉能力，因此提供内置图片测试用于诊断；测试状态不作为自动入账的额外门槛。
...
- 用户可通过内置模拟付款图测试模型；通过后标记为“视觉已验证”，该标记仅表示能力测试状态。
...
资产页面是总览和设置共用的财务资产工作区，入口来源决定返回路径：从总览进入返回总览，从设置进入返回设置，不新增第五个 Tab。

### 数据口径

- 总存款：银行卡、储蓄账户、定期存款的人民币正余额。
- 流动资金：总存款、现金和微信/支付宝等支付账户的人民币正余额。
- 投资资产：投资账户的人民币正余额。
- 其他资产：房产、车辆、贵金属、应收款、保险、收藏品等非账户资产估值。
- 总资产：所有纳入净资产的人民币正资产。
- 总负债：负余额账户和贷款、车贷、消费贷等负债项目的绝对值。
- 净资产：总资产减总负债。

账户和其他资产都支持“计入净资产”开关、归档和详情编辑。资产估值更新保留历史记录；账户余额校准单独保存调整记录。两类记录都不写入 `LedgerTransaction`，因此不会改变月度收入、支出或预算统计。银行转投资仍是转账，不是支出。

第一阶段只汇总人民币。外币账户和资产保留原币金额，并在资产页面提示未计入人民币汇总；不接行情或汇率 API，不增加后端和付费依赖。

### 备份与迁移

JSON 备份 schema 为 `3`，除账单、账户、分类、预算和周期提醒外，还包含资产、估值历史、余额校准、归档状态、净资产开关及时间元数据。恢复按各类记录 ID 去重，不覆盖现有数据；旧 schema 缺少资产字段时按空集合处理，旧账户缺少资产分组时按账户类型自动映射。
